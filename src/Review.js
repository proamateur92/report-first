...
const Review = () => {
  ...
  const renderPoints = () => {
    ...
    for (let i = 0; i < points; i++) {
      if (point <= i) {
        result.push(<div className='points purple' data-num={i + 1} onMouseEnter={handleEnter}></div>);
      } else {
        result.push(<div className='points dark-purple' data-num={i + 1} onMouseEnter={handleEnter}></div>);
      }
    }
    return result;
  };
